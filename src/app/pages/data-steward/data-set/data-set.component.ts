...
import { Hierarchy, IDataset, IDatasetLocationRequest, IDataSource, IEnum, IHierarchy, NullableGuidId } from 'src/app/shared/api/api';
...
export class DataSetComponent implements OnInit {
  ...
  refinementLevel$: Observable<IEnum[]>;
  ...
  ngOnInit() {
    ...
    this.data.status = 0;
    this.data.hierarchy = this.data.hierarchy ?? new Hierarchy()
  ...
  getDataLocation() {
    const locationRequest: IDatasetLocationRequest = {
      name: this.data.name,
      hierarchy: new NullableGuidId({id: this.data.hierarchy.id}),
    };
